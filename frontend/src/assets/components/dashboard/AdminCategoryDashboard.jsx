...
const AdminCategoryDashboard = () => {
  ...
  // 🎨 --- Theme Styles (Professional Dark) ---
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-[#1E1E1E] p-6 rounded-xl shadow-lg mb-12 border border-[#2F2F2F]">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <input
                type="text"
                placeholder="Enter Category Name (e.g., Plumbing, Electrical)"
                value={categoryName}
                onChange={(e) => setCategoryName(e.target.value)}
                className={darkInputClass}
                onKeyPress={(e) => e.key === "Enter" && handleAddOrEditCategory()}
              />
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {categories.map((cat) => (
              <div
                key={cat._id}
                className="bg-[#1E1E1E] shadow-lg rounded-xl p-6 border-t-4 border-[#FF8C42] hover:shadow-[#FF8C42]/30 transition-all duration-300"
              >
                  ...
                  <div className="flex gap-2">
                    ...
                    {editingSub.catId === cat._id ? (
                      ...
                      <button
                        ...
                        className="bg-[#3B82F6] hover:bg-[#2563EB] text-white p-2 rounded-lg"
                      >
                        <FaPlus />
                      </button>
                    )}
                ...
                <div className="flex flex-wrap gap-3 max-h-40 overflow-y-auto custom-scrollbar-dark">
                  {cat.subcategories.map((sub) => (
                    <div
                      key={sub._id}
                      className="bg-[#2C2C2C] px-3 py-1.5 rounded-full flex items-center gap-2 text-sm border border-[#3A3A3A]"
                    >
                      <span className="text-white capitalize">{sub.name}</span>
                      <button
                        onClick={() => handleEditSubcategory(cat._id, sub)}
                        className="text-[#EAB308] hover:text-yellow-300"
                      >
                  ...
                  {cat.subcategories.length === 0 && (
                    <p className="text-gray-500 italic text-sm">No subcategories defined.</p>
                  )}
                </div>
              </div>
            ))}
          ...
          {categories.length === 0 && (
            <p className="text-gray-500 mt-16 text-center text-xl">
              No categories found. Use the form above to get started.
            </p>
          )}
        </div>
      </div>
